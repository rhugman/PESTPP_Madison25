...
def plot_ies_results(m_d,case="at",tag="",piter=None):
    ...
    ptoe = pst.ies.get("obsen",piter)
    proe = pst.ies.obsen0
    phivals = pst.ies.phimeas.iloc[piter,6:]
    ...
    prarr = np.zeros((kobs.i.max()+1,kobs.j.max()+1))
    prarr[kobs.i,kobs.j] = np.log10(proe.loc[ptoe.index[0],kobs.obsnme])
    ptarr = np.zeros((kobs.i.max()+1,kobs.j.max()+1))
    ptarr[kobs.i,kobs.j] = np.log10(ptoe.loc[ptoe.index[0],kobs.obsnme])
    fig,axes = plt.subplots(1,2,figsize=(8,4))
    cb = axes[0].imshow(prarr,vmin=ptarr.min())
    plt.colorbar(cb,ax=axes[0])
    axes[0].set_title("prior real {0} {1}".format(ptoe.index[0],tag))
    cb = axes[1].imshow(ptarr,vmax=prarr.max())
    plt.colorbar(cb,ax=axes[1])
    axes[1].set_title("posterior real {0} {1}".format(ptoe.index[0],tag))

    prarr = np.zeros((kobs.i.max()+1,kobs.j.max()+1))
    prarr[kobs.i,kobs.j] = np.log10(proe.loc[ptoe.index[1],kobs.obsnme])
    ptarr = np.zeros((kobs.i.max()+1,kobs.j.max()+1))
    ptarr[kobs.i,kobs.j] = np.log10(ptoe.loc[ptoe.index[1],kobs.obsnme])
    fig,axes = plt.subplots(1,2,figsize=(8,4))
    cb = axes[0].imshow(prarr,vmin=ptarr.min())
    plt.colorbar(cb,ax=axes[0])
    axes[0].set_title("prior real {0} {1}".format(ptoe.index[1],tag))
    cb = axes[1].imshow(ptarr,vmax=prarr.max())
    plt.colorbar(cb,ax=axes[1])
    axes[1].set_title("posterior real {0} {1}".format(ptoe.index[1],tag))
